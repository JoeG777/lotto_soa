...
def calculate_winning_class(lotto_draw: LottoDraw, bet: Bet) -> int:
    matching_winning_numbers = len(
        set(lotto_draw.winning_numbers).intersection(set(bet.winning_numbers))
    )
    matching_super_number = len(
        set(lotto_draw.super_number).intersection(set(bet.super_number))
    )
